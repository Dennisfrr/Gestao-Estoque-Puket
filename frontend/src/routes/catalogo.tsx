...
function CatalogPage() {
    ...
    try {
      ...
      setBlingStatus(Object.fromEntries(items.map(product => {
        ...
        return [product.id, item?.existe
          ? { state: "exists", ...(item.id !== undefined ? { id: item.id } : {}) } satisfies BlingState
  ...
  const load = async (nextPage = 0) => { setLoading(true); setError(""); blingCheckId.current++; setCheckingBling(false); setBlingStatus({}); try { const response = await searchCatalog({ q: query, linhas: line, grupos: group, solucoes: solution, pagina: nextPage, limite: 24 }); setProducts(response.produtos); setPage(response.pagina); setHasMore(response.temMais); setSelected([]); setSendState("idle"); setBlingFilter("all"); } catch (reason) { setError(reason instanceof Error ? reason.message : "Não foi possível carregar o catálogo."); } finally { setLoading(false); } };
  ...
  const hasBlingCheck = Object.keys(blingStatus).length > 0;
  ...
  return <AppLayout><div className="mb-7 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-extrabold uppercase tracking-[.18em] text-primary">Catálogo integrado</p><h1 className="mt-1 font-display text-3xl font-extrabold sm:text-5xl">Catálogo Puket</h1><p className="mt-2 text-muted-foreground">Encontre, selecione e envie produtos completos para o Bling.</p></div>{selected.length > 0 && <Button className="h-12 rounded-full px-6" onClick={() => setSendState("review")}><Send/>Enviar {selected.length} para o Bling</Button>}</div>
    ...
    {!loading && products.length > 0 && <section className="mb-6 flex flex-col gap-3 rounded-2xl border bg-card p-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex flex-wrap items-center gap-2">{hasBlingCheck ? <><BlingFilterButton active={blingFilter === "all"} onClick={() => setBlingFilter("all")}>Todos <strong>{products.length}</strong></BlingFilterButton><BlingFilterButton active={blingFilter === "exists"} onClick={() => setBlingFilter("exists")} tone="success">No Bling <strong>{checkingBling ? "…" : existingCount}</strong></BlingFilterButton><BlingFilterButton active={blingFilter === "missing"} onClick={() => setBlingFilter("missing")} tone="warning">Não enviados <strong>{checkingBling ? "…" : missingCount}</strong></BlingFilterButton></> : <p className="px-2 text-sm text-muted-foreground">A situação no Bling só será consultada quando você solicitar.</p>}</div><Button variant={hasBlingCheck ? "ghost" : "outline"} className="rounded-full" onClick={() => void checkBling(products)} disabled={checkingBling}>{checkingBling ? <Loader2 className="animate-spin"/> : <RefreshCw/>}{checkingBling ? "Consultando Bling" : hasBlingCheck ? "Atualizar situação" : "Consultar no Bling"}</Button></section>}
...
function BlingStatus({ status }: { status: BlingState | undefined }) {
  if (!status) return null;
  if (status.state === "checking") return <span className="absolute right-4 top-4 z-10 flex items-center gap-1.5 rounded-full bg-background/95 px-3 py-1.5 text-xs font-bold text-muted-foreground shadow"><Loader2 className="h-3.5 w-3.5 animate-spin"/>Consultando</span>;
